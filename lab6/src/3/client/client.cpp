...
void* sendToServer(const Server& addressStruct,const Parametrs& originalParametrs,const uint64_t begin,const uint64_t end)
{
    ...
    std::pair<sockaddr_in,int> pairConnect = GetConnectToServer(htons(addressStruct.port),*((unsigned long *)hostname->h_addr));
    ...
    SendMessageToServer(pairConnect,task,sizeof(uint64_t) * 3);

    uint64_t answer = GetMessageToServer(pairConnect);
    close(pairConnect.second);
